...
# general
parser.add_argument('--data-dir', default='/home/javad/Datasets/YTF/aligned_images_DB/', help='')
parser.add_argument('--image-size', type=int, default=112, help='')
parser.add_argument('--gpu', type=int, default=1, help='')
parser.add_argument('--det-prefix', type=str, default='/home/javad/insightface/RetinaFace/model/R50', help='')
parser.add_argument('--output', default='./ytf.bin', help='path to save.')
...
issame_list = []
pp = 0
for line in open('pairs.txt', 'r'):
  pp+=1
  if pp%100==0:
    print('processing', pp)
  line = line.strip().split()[2:5]
  # print(line)
  assert len(line)==3
  try:
    path1 = os.path.join(args.data_dir, line[0])
    path1 = os.path.join(path1, random.choice(os.listdir(path1)).replace("pickle", "jpg"))
    # print(path1)
    path2 = os.path.join(args.data_dir, line[1])
    path2 = os.path.join(path2, random.choice(os.listdir(path2)).replace("pickle", "jpg"))
    # print(path2)
  except:
    continue
  # start = time.time()
